        ...
        <div style={{ backgroundColor: 'white', border: '1px solid gray', borderRadius: '5px' }}>
          {editName ? <RenameItem col_name={name} stream_id={stream_id} cancel={cancelEditHandler} refreshName={refreshColumn} /> :
            < Stack direction="row" sx={{ justifyContent: "space-between" }}>
              <Typography id="transition-modal-title" variant="h5" component="h2" mt={1} mb={1} sx={{ color: "#37474f", marginLeft: "10px", borderBottom: "1px solid gray" }}>
                {name}
            ...
            </Stack>}
          {
            tasks?.map((task: { id: any; name: any; stream_id: any; }) => {
              if (task.stream_id === stream_id) { return (<TaskCard key={task.id} task_name={task.name} refresh_tasks={refresh} task_id={task.id} column_data={streams} current_col_id={stream_id} />) }
            })
          }
          <CreateModal btnText='Add Task' rows={2} setTags={setTask} color='success' width='300px'
